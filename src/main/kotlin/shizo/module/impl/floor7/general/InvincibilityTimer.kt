...
import shizo.utils.capitalizeFirst
import shizo.utils.createSkullStack
...
object InvincibilityTimer : Module(
    ...
    private val invincibilityAlert by BooleanSetting(
        "Invincibility Alert",
        true,
        desc = "Plays a sound when you get invincibility."
    )
    private val invincibilityAnnounce by BooleanSetting(
        "Announce Invincibility",
        true,
        desc = "Announces when you get invincibility."
    )
    private val showWhen by SelectorSetting(
        "Show",
        "Always",
        listOf("Always", "Any", "When Active", "On Cooldown"),
        "Controls when invincibility items are shown."
    )
    private val equippedMaskColor by ColorSetting(
        "Equipped Mask",
        Colors.MINECRAFT_DARK_PURPLE,
        desc = "Color of the equipped mask in the HUD. (Bonzo/Spirit)"
    )
    ...
    private val hud by HUD(name, "Shows the invincibility time in the HUD.") { example ->
        ...
        visibleTypes.forEachIndexed { index, type ->
            ItemStateRenderer.Companion.draw(this, type.itemStack, 0, -1 + index * 14)
            val y = index * 14 + 3
    ...
    private val showOnlyInBoss by BooleanSetting(
        "Show In Boss",
        false,
        desc = "Only shows invincibility timers during dungeon boss fights."
    )
