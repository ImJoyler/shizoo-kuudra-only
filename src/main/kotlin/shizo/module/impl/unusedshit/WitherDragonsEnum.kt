...
import shizo.module.impl.unusedshit.WitherDragons.currentTick
import shizo.module.impl.unusedshit.WitherDragons.sendSpawning
...
enum class WitherDragonsEnum(
    ...
    fun setAlive(entityId: UUID?) {
        ...
        timesSpawned++
        spawnedTime = WitherDragons.currentTick
        isSprayed = false

        if (WitherDragons.sendSpawned && WitherDragons.enabled) {
    ...
    fun setDead(realTime: Boolean) {
        ...
        entityUUID = null
        DragonCheck.lastDragonDeath = this

        if (WitherDragons.priorityDragon == this) WitherDragons.priorityDragon = null

        if (WitherDragons.sendTime && WitherDragons.enabled && realTime)
    ...
    companion object {
        fun reset(soft: Boolean = false) {
            ...
            WitherDragons.priorityDragon = null
...
fun handleSpawnPacket(particle: ClientboundLevelParticlesPacket) {
    ...
    if (dragons.isNotEmpty() && (dragons.size == 2 || spawned >= 2) && WitherDragons.priorityDragon == null)
        WitherDragons.priorityDragon = DragonPriority.findPriority(dragons).also { dragon ->
            if (WitherDragons.dragonTitle && WitherDragons.enabled) alert("§${dragon.colorCode}${dragon.name} is spawning!", true)
            if (WitherDragons.dragonPriorityToggle && WitherDragons.enabled) modMessage("${dragons.joinToString(", ") { "§${it.colorCode}${it.name}" }}§r -> §${dragon.colorCode}${dragon.name} §7is your priority dragon!")
        }
}
