    ...
    init {
        ...
        onReceive<ClientboundSetEntityDataPacket> {
            if (hideArcherBoneMeal && DungeonUtils.inDungeons) {
                val item = this.packedItems.find { it.id == 8 }?.value as? ItemStack ?: return@onReceive
                if (!item.isEmpty && item.item == Items.BONE_MEAL) mc.execute {
                    mc.level?.removeEntity(this.id, Entity.RemovalReason.DISCARDED)
                }
            }
        }
