...
object MapScanner {
    ...
    var bloodpos: Vec2i? = null
        ...
        private fun scanMimic(world: Level) {
            if (allRooms.values.any { it.hasMimic }) return
            ...
            val chestsPerRoom = HashMap<MapRoom, Int>()

            for (x in (pX - renderDist)..(pX + renderDist)) {
                for (z in (pZ - renderDist)..(pZ + renderDist)) {
                    val chunk = world.getChunk(x, z, ChunkStatus.FULL, false) ?: continue

                    for (pos in chunk.blockEntitiesPos) {
                        if (world.getBlockState(pos).block == Blocks.TRAPPED_CHEST) {
                            getRoomAt(pos)?.let { room ->
                                chestsPerRoom[room] = chestsPerRoom.getOrDefault(room, 0) + 1
                            }
                        }
                    }
                }
            ...
            for ((room, count) in chestsPerRoom) {
                if (count > room.data.trappedChests) {
                    room.hasMimic = true
                }
            }
        }

        private fun getRoomAt(pos: BlockPos): MapRoom? {
            val gridX = (pos.x + 185) / 16
            val gridZ = (pos.z + 185) / 16

            if (gridX in 0..10 && gridZ in 0..10) {
                val tile = list[gridX * 11 + gridZ]
                if (tile is MapRoom.RoomTile) {
                    return tile.owner
                }
            }
            return null
