package shizo.utils.network.hypixelapi

import com.google.gson.annotations.SerializedName
import net.minecraft.nbt.NbtAccounter
import net.minecraft.nbt.NbtIo
import shizo.utils.capitalizeWords
import shizo.utils.magicalPower
...
object HypixelData {
    ...
    data class MemberData(
        ...
        val level get() = (leveling.experience / 100).toInt()
        val kuudraCompletions get() = crimsonIsle.kuudraCompletedTiers
        ...
        val allItems get() = ((inventory?.invContents?.itemStacks ?: emptyList()) +
                (inventory?.eChestContents?.itemStacks ?: emptyList()) +
                (inventory?.backpackContents?.flatMap { it.value.itemStacks } ?: emptyList()) +
                (inventory?.wardrobeContents?.itemStacks ?: emptyList()) +
                (inventory?.invArmor?.itemStacks ?: emptyList()))
    ...
    data class CrimsonIsle(
        ...
        val kuudraCompletedTiers: Map<String, Int> = emptyMap()
    )
