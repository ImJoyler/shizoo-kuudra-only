...
import shizo.utils.JsonResourceLoader
import shizo.utils.Vec2
import shizo.utils.equalsOneOf
import shizo.utils.skyblock.dungeon.tiles.RoomData
...
import shizo.utils.skyblock.dungeon.tiles.RoomComponent
...
object ScanUtils {
    ...
    val roomList: Set<RoomData> = JsonResourceLoader.loadJson("/assets/shizo/rooms.json", setOf())
    ...
    init {
        on<TickEvent.End> {
            ...
            scanRoom(roomCenter)?.let { room -> if (room.rotation != Rotations.NONE) RoomEnterEvent(room).postAndCatch() } ?: run {
                ...
                //Shizo.logger.error("Unable to determine room at $roomCenter core: ${getCore(roomCenter)}")
            }
        }
        ...
        on<RoomEnterEvent> {
            ...
            Shizo.logger.error("${room?.data?.name} - ${room?.rotation} || clay: ${room?.clayPos}")
        }
    ...
    fun scanRoom(vec2: Vec2): Room? {
        ...
        return getCoreAtHeight(vec2, roomHeight, chunk).let { core ->
            coreToRoomData[core]?.let { roomData ->
                Room(
                    data = roomData,
                    roomComponents = findRoomComponentsRecursively(vec2, roomData.cores, roomHeight, level)
                )
