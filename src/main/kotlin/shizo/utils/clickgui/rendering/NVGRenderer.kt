...
import shizo.utils.Color.Companion.alpha
import shizo.utils.Color.Companion.blue
import shizo.utils.Color.Companion.green
import shizo.utils.Color.Companion.red
...
object NVGRenderer {
    ...
    val defaultFont by lazy { Font("Default", mc.resourceManager.getResource(ResourceLocation.parse("shizo:font.ttf")).get().open())}
    ...
    private val vg: Long by lazy {
        val pointer = nvgCreate(NVG_ANTIALIAS or NVG_STENCIL_STROKES)
        require(pointer != -1L) { "Failed to initialize NanoVG" }
        pointer
    }
    ...
    fun customRect(x: Float, y: Float, w: Float, h: Float, color: Int, radTopLeft: Float, radTopRight: Float, radBottomRight: Float, radBottomLeft: Float) {
        nvgBeginPath(vg)
