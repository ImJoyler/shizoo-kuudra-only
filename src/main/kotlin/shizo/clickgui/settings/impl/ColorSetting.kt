...
import shizo.utils.Color.Companion.hsbMax
...
import shizo.utils.clickgui.rendering.Gradient
...
class ColorSetting(
    ...
    override fun render(x: Float, y: Float, mouseX: Float, mouseY: Float): Float {
        ...
        if (isModern) {
            val previewX = x + width - 40f
            val previewY = y + defaultHeight / 2f - 8f
            NVGRenderer.rect(previewX, previewY, 25f, 16f, value.rgba, 3f)
            NVGRenderer.hollowRect(previewX, previewY, 25f, 16f, 1f, Color(255, 255, 255, 0.2f).rgba, 3f)
        } else {
        ...
        val sbPointer = Pair((x + 6f + animatedSat * (width - 12f)), (y + 38f + (1 - animatedBright) * 170))
        ...
        val huePos = x + 6f + hueSliderAnim.get(hueSliderPrev, value.hue, false) * (width - 13f) to y + 219f
        ...
        if (allowAlpha) {
            ...
            val alphaPos = Pair((x + 6f + alphaSliderAnim.get(alphaSliderPrev, value.alphaFloat, false) * (width - 15f)), y + 240f)
        ...
        if (isModern) {
            NVGRenderer.rect(rectX, y + actualHeight - 28f, width / 2, 24f, Color(0, 0, 0, 0.4f).rgba, 3f)
            NVGRenderer.hollowRect(rectX, y + actualHeight - 28f, width / 2, 24f, 1f, Color(255, 255, 255, 0.2f).rgba, 3f)
        } else {
    ...
    override fun mouseClicked(mouseX: Float, mouseY: Float, click: MouseButtonEvent): Boolean {
        ...
        section = when {
            isAreaHovered(lastX + 6f, lastY + 36f, width - 12f, 170f, true) -> 0
            isAreaHovered(lastX + 6f, lastY + 212f, width - 12f, 15f, true) -> 1
            isAreaHovered(lastX + 6f, lastY + 232, width - 12f, 15f, true) && allowAlpha -> 2
            else -> null
        }
    ...
    override fun keyPressed(input: KeyEvent): Boolean {
        return if (extended) textInputHandler.keyPressed(input) else false
    }

    override fun keyTyped(input: CharacterEvent): Boolean {
        return if (extended) textInputHandler.keyTyped(input) else false
    }
    ...
    override val isHovered: Boolean
        get() {
            val isModern = ClickGUIModule.theme.value == 1
            return if (isModern) {
                isAreaHovered(lastX + width - 40f, lastY + defaultHeight / 2f - 8f, 25f, 16f, true)
            } else {
                isAreaHovered(lastX + width - 40f, lastY + defaultHeight / 2f - 10f, 34f, 20f, true)
            }
        }
