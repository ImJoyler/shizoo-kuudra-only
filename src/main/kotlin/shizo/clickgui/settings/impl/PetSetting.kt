...
class PetConfigSetting(
    name: String,
    val mobId: Int,
    defaultData: PetData = PetData(),
    description: String = "Open the 3D Pet Wardrobe"
) : RenderableSetting<PetData>(name, description), Saving {

    override val default: PetData = defaultData
    override var value: PetData = default.copy()
    ...
    override fun write(gson: Gson): JsonElement = JsonObject().apply {
        ...
        addProperty("dist", value.dist)
        addProperty("side", value.side)
        addProperty("height", value.height)
        addProperty("yaw", value.yaw)
    }

    override fun read(element: JsonElement, gson: Gson) {
        if (element !is JsonObject) return
        value.size = element.get("size")?.asFloat ?: value.size
        value.pitch = element.get("pitch")?.asFloat ?: value.pitch
        value.dist = element.get("dist")?.asFloat ?: value.dist
        value.side = element.get("side")?.asFloat ?: value.side
        value.height = element.get("height")?.asFloat ?: value.height
        value.yaw = element.get("yaw")?.asFloat ?: value.yaw
    }
}